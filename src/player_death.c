...
/*   Updated: 2023/08/11 14:42:08 by ycyr-roy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	menu_press(void)
{
	...
	data = get_data();
	if ((!data->player_alive && data->tiles->death_screen[0]->enabled) \
		|| (data->win && data->tiles->win_screen[0]->enabled))
	{
		data->player_alive = 1;
		...
		|| data->tiles->win_screen[1]->enabled)
		exit(EXIT_SUCCESS);
}
